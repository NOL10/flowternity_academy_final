...
export async function getDb() {
  ...
  if (!indexesCreated) {
    try {
      ...
      await db.collection('leadership_metrics').createIndex({ user_id: 1 });
      await db.collection('leadership_metrics').createIndex({ user_id: 1, sport_id: 1 });
      await db.collection('leadership_metrics').createIndex({ sport_id: 1, recorded_at: -1 });
      indexesCreated = true;
    } catch (e) {
      // Non-fatal
